...
const Color = "#F9D978"; // If set to "default," it will use the theme's cursor color.
...
createCursorHandler({
	...
	onStarted: (editor) => {
		...
		let color = Color;
		if (color == "default") {
			color = getComputedStyle(document.querySelector("body>.monaco-workbench")).getPropertyValue("--vscode-editorCursor-background").trim();
		}
